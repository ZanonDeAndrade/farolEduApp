...
const resetCounters = async () => {
    ...
    for (const c of counters) {
        await firebase_1.firestore.collection("counters").doc(c).set({ current: 0 }, { merge: true });
    }
};
const migrateUsers = async () => {
    ...
    for (const user of users) {
        const userRef = firebase_1.firestore.collection(USERS).doc(String(user.id));
        batch.set(userRef, {
            id: user.id,
            name: user.name,
            email: user.email,
        ...
        if (user.teacherProfile) {
            const profileRef = firebase_1.firestore.collection(TEACHER_PROFILES).doc(String(user.teacherProfile.id));
        ...
        if (user.teacherClasses?.length) {
            for (const cls of user.teacherClasses) {
                const classRef = firebase_1.firestore.collection(TEACHER_CLASSES).doc(String(cls.id));
                batch.set(classRef, {
                    id: cls.id,
...
const migrateSchedules = async () => {
    ...
    const chunks = (arr, size) => arr.reduce((acc, _, idx) => (idx % size === 0 ? [...acc, arr.slice(idx, idx + size)] : acc), []);
    let total = 0;
    for (const batchItems of chunks(schedules, 400)) {
        const batch = firebase_1.firestore.batch();
        for (const sched of batchItems) {
            const ref = firebase_1.firestore.collection(SCHEDULES).doc(String(sched.id));
            batch.set(ref, {
                id: sched.id,
...
main()
    ...
    if (firebase_admin_1.default.apps.length) {
        await firebase_1.firestore.terminate?.().catch(() => { });
    }
});

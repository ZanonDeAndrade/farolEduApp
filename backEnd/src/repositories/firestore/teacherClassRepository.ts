...
const normalizeModality = (value?: string | null) =>
  (value ?? "ONLINE").toString().trim().toUpperCase() || "ONLINE";

const normalizePriceCents = (price?: number | null, priceCents?: number | null) => {
...
export class FirestoreTeacherClassRepository implements ITeacherClassRepository {
  async createTeacherClass(input: CreateTeacherClassInput): Promise<TeacherClass> {
    ...
    const now = new Date();
    const id = await getNextId(TEACHER_CLASSES);
    const priceCents = normalizePriceCents(input.price as number | null, input.priceCents);
    const payload = {
      id,
      ...
      durationMinutes:
        Number.isFinite(input.durationMinutes) && input.durationMinutes
    ...
    await getCollection(TEACHER_CLASSES).doc(String(id)).set(payload);
    return mapClass(payload, id);
  }

  async updateTeacherClassByTeacher(
    teacherId: number,
    classId: number,
    ...
    const ref = getCollection(TEACHER_CLASSES).doc(String(classId));
    const snap = await ref.get();
    if (!snap.exists) return null;
    const existing = mapClass(snap.data()!, classId);
    if (existing.teacherId !== teacherId) return null;

    const priceCents =
      data.price !== undefined || data.priceCents !== undefined
        ? normalizePriceCents(data.price as number | null, data.priceCents)
        : existing.priceCents;
    const price =
  ...
  async deleteTeacherClassByTeacher(teacherId: number, classId: number): Promise<boolean | null> {
    if (!Number.isFinite(teacherId) || !Number.isFinite(classId)) {
      throw new Error("INVALID_IDS");
    }
    const ref = getCollection(TEACHER_CLASSES).doc(String(classId));
  ...
  async getTeacherClassesByTeacher(teacherId: number): Promise<TeacherClass[]> {
    if (!Number.isFinite(teacherId)) throw new Error("INVALID_TEACHER_ID");
    const snap = await getCollection(TEACHER_CLASSES)
  ...
  async getPublicTeacherClasses(filters: PublicTeacherClassFilters): Promise<Array<TeacherClass & {
    ...
    let ref: FirebaseFirestore.Query = getCollection(TEACHER_CLASSES).where("active", "==", true);
    ...
    const filtered = classes.filter(item => {
      ...
      if (lowerCity && teacher?.teacherProfile) {
        ...
        if (!combined.includes(lowerCity)) return false;
      }
      if (lowerTeacherName && teacher?.name) {
        if (!teacher.name.toLowerCase().includes(lowerTeacherName)) return false;
      }
      if (lowerQuery) {
        const text = `${item.title} ${item.subject ?? ""} ${item.description ?? ""} ${teacher?.name ?? ""}`.toLowerCase();
        if (!text.includes(lowerQuery)) return false;
      }
      return true;
    });
